...
public:
	int h; // 树高
    BPT(int order=3):_node_num(1), h(1), _order(order), _size(0){
       // _root = new BPTNode<T>();
	...
	int serialize(const char *path) {
		...
		for (BPTNode<T> *layer = _root; layer != NULL; layer = layer->child[0]) {
			assert(i < h);
			int num = 0;
	...
	int deserialize(const char *path) {
		...
		_node_num = meta.BPnode_num; _size = meta.BPsize; _order = meta.BPorder; h = meta.BPh;
		Array<T>layernum(h);
		...
		for (int i = 0; i < h; i++) {
			...
			for (int k = 0; k < prev->key.size(); k++)prev->child.insert(NULL);
			for (int j = 0; j < layernum[i] - 1 ; j++) {
				BPTNode<T> *next = new BPTNode<T>(_order);
				next->key.deserialize(fp);
				for (int k = 0; k < next->key.size();k++)next->child.insert(NULL);
				next->prev = prev;
				prev->next = next;
				prev = next;
			}
